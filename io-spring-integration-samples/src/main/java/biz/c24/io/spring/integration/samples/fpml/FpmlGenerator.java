	...
	public static void main(String[] args) {
		if (args.length == 1) {
			...
			try {
				System.out.println("Parsing... ");
				Source source = element.getModel().source();
				source.setInputStream(new FileInputStream(args[0]));
				ComplexDataObject bean = source.readObject(element);
				...
				for (int i = 0; i < 10000; i++) {
					TradeConfirmed fpML = randomizeFpML(tradeConfirmed);
					Number number = IOXPathFactory.getInstance("//amount")
							.getNumber(fpML);
			...
			System.out
					.println("Usage: java org.fpml.x2010.fpml49.FpMLElement <filename>");
	}
